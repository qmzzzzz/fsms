...
├── jest.config.js              # 测试配置（覆盖率棘轮：全局 分支56/函数64/语句70/行72，安全关键模块另设独立阈值，逐档上调至80%）
└── .env.example                # 环境变量模板
```
...
| [docs/adr/](./docs/adr/README.md)              | 架构决策记录：登录 ECDH 选型、审计链哈希、密钥管理、单进程锁假设 |
...
npm run test:coverage    # 运行测试并输出覆盖率（全局阈值：分支56/函数64/语句70/行72；安全关键模块另设独立阈值，棘轮逐档上调至80%）
